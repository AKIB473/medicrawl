...
class MIMSScraper(BaseAdvancedScraper):
    name = 'mims'
    ...
    async def _scrape_drug_page(self, url: str) -> Drug | None:
        ...
        return Drug(source='mims', source_url=url, brand_name=title, generic_name=generic, atc_code=atc, manufacturer=Manufacturer(name=manufacturer_name) if manufacturer_name else None, dosage_form=fields.get('dosage form', fields.get('form', '')), strength=fields.get('strength', ''), indications=_split(sections.get('indications', sections.get('indication', ''))), contraindications=_split(sections.get('contraindications', '')), side_effects=_split(sections.get('adverse effects', sections.get('side effects', ''))), warnings=_split(sections.get('special precautions', sections.get('warnings', ''))), interactions=_split(sections.get('drug interactions', sections.get('interactions', ''))), dosage=sections.get('dosage', sections.get('dosage/direction for use', '')), mechanism_of_action=sections.get('action', sections.get('mechanism of action', '')), pharmacokinetics={'description': sections.get('pharmacokinetics', '')} if sections.get('pharmacokinetics') else None, pregnancy_category=sections.get('pregnancy', fields.get('pregnancy category', '')), storage=sections.get('storage', fields.get('storage', '')), description=sections.get('description', ''), extra={'jsonld': jsonld, 'mims_class': fields.get('mims class', ''), 'presentation': sections.get('presentation/packing', ''), 'overdosage': sections.get('overdosage', ''), 'caution': sections.get('cautions', ''), 'sections': sections, 'fields': fields})
